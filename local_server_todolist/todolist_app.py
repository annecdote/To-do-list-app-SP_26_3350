...
def fetch_tasks(conn, project_filter, sort_by, search_term, show_filter):
    ...
    if search_term:
        ...
        tasks = [
            ...
            if needle in row["task"].casefold() or needle in (row["notes"] or "").casefold()
        ]
    ...
    def do_GET(self):
        ...
        with connect_db() as conn:
            ...
            if parsed.path == "/toggle":
                ...
                if task:
                    conn.execute(
                        ...
                        (0 if task["completed"] else 1, tid),
                    )
                    conn.commit()
                self.redirect_home(project_filter, sort_by, search_term, show_filter)
                return
            ...
            for t in active_tasks:
                ...
                due_badge = ""
                if t["due_date"]:
                    due_badge = f"<span class='task-tag due'>Due {html.escape(t['due_date'])}</span>"

                notes_html = ""
                if t["notes"] and not is_editing:
                    notes_html = f"<div class='task-notes'>{html.escape(t['notes'])}</div>"
                ...
                <div class="task-card">
                    <div class="task-left">
                        <a class="checkbox {checked}" href="/toggle?id={t['id']}&project={project_filter}&sort={sort_by}&q={html.escape(search_term)}&show={show_filter}" title="Toggle complete"></a>
                        <div class="task-main">
                    ...
                    <div class="task-actions">
                        ...
                        <a class="icon-btn" href="/delete?id={t['id']}&project={project_filter}&sort={sort_by}&q={html.escape(search_term)}&show={show_filter}" title="Delete">🗑</a>
                        <a class="icon-btn" href="/move?id={t['id']}&dir=up&project={project_filter}&sort={sort_by}&q={html.escape(search_term)}&show={show_filter}" title="Move up">⌃</a>
                        <a class="icon-btn" href="/move?id={t['id']}&dir=down&project={project_filter}&sort={sort_by}&q={html.escape(search_term)}&show={show_filter}" title="Move down">⌄</a>
                    </div>
                </div>
                """)
            ...
            for row in projects:
                ...
                project_links.append(
                    f"<a class='{active}' href='/{build_query(p, sort_by, None, search_term, show_filter)}'><span>📁 {html.escape(p)}</span><span>{row['open_count'] or 0}</span></a>"
                )
            ...
            <head>
                ...
                <style>
                    * {{
                    ...
                    .outline-btn, .project-add-form button {{
                        display: block;
                        width: 100%;
                        margin-top: 12px;
                        padding: 14px 18px;
                    ...
                    .add-form {{
                        display: grid;
                        grid-template-columns: 1.6fr 160px 145px 135px 150px 120px;
                        gap: 12px;
                        flex: 1;
                        min-width: 760px;
                    }}
                    ...
                    .edit-form {{
                        display: grid;
                        grid-template-columns: 1.4fr 1.2fr 150px 140px 140px 150px auto;
                        gap: 12px;
                        width: 100%;
                    }}
                ...
                <div class="window">
                    ...
                    <div class="app">
                        <aside class="sidebar">
                            ...
                            <a class="nav-item {'active' if project_filter == 'Inbox' else ''}" href="/{build_query('Inbox', sort_by, None, search_term, show_filter)}"><span>▢ Inbox</span><span>{stats['open']}</span></a>
                            <a class="nav-item {'active' if show_filter == 'active' else ''}" href="/{build_query(project_filter, sort_by, None, search_term, 'active')}"><span>🗓 Active</span><span>{stats['open']}</span></a>
                            <a class="nav-item {'active' if show_filter == 'completed' else ''}" href="/{build_query(project_filter, sort_by, None, search_term, 'completed')}"><span>✓ Completed</span><span>{stats['completed']}</span></a>
        ...
        with connect_db() as conn:
            if parsed.path == "/add":
                ...
                if task:
                    ...
                    conn.execute(
                        """
                        INSERT INTO tasks (task, notes, project, label, priority, due_date, completed, created_at, position)
                        VALUES (?, '', ?, ?, ?, ?, 0, ?, ?)
                        """,
                        (
                            task,
                            project if project else "Work",
                            label if label in LABELS else "",
                ...
                if task:
                    ...
                    if new_text:
                        conn.execute(
                            """
                            UPDATE tasks
                            SET task = ?, notes = ?, project = ?, label = ?, priority = ?, due_date = ?
                            WHERE id = ?
                            """,
                            (
                                new_text,
                                new_notes,
                                new_project if new_project else task["project"],
                                new_label if new_label in LABELS else "",
                                new_priority if new_priority in PRIORITIES else task["priority"],
                                new_due,
                                tid,
                            ),
                        )
...
def add_extra_routes():
    original_get = TodoHandler.do_GET

    def extended_get(self):
        parsed = urlparse(self.path)
        query = parse_qs(parsed.query)
        project_filter = query.get("project", ["Inbox"])[0]
        sort_by = query.get("sort", ["manual"])[0]
    ...
    TodoHandler.do_GET = extended_get
